...
public class MissionTemplateManagementImpl implements MissionTemplateManagement {
  ...
  public MissionHeader createMissionFromMission(
    ...
    if(MissionTemplate != null)
    {
      ...
      lmissionHeader = (MissionHeader) TransferFields.transferFields(lmissionHeader, pMissionTemplate);
       ...
       */
      lmissionHeader = missionHeaderValidate.validateAuctioneerCode(lmissionHeader, lmissionHeader.getAuctionnerCode());
      ...
      {
        lmissionHeader.setDescription(pMissionTemplate.getCode() + " " + lmissionHeader.getMasterContactNo().getFullName());
      }
      lmissionHeader = missionHeaderValidate.validateResponsibilityCenter(lmissionHeader, lmissionHeader.getResponsibilityCenter());
     ...
     */
      ...
      {
        LawyerBusiness lLawyerBus = Beans.get(LawyerBusinessRepository.class).all().filter("self.no = ?", pLawyerBusNo).fetchOne();
        if(lLawyerBus != null)
        {
          //TODO REVOIR LE CODE
          ...
          {
            case "Procedure RJ":
              lmissionHeader = missionHeaderValidate.validateMasterContactNo(lmissionHeader, lLawyerBus.getDebtorContactNo());
              break;
            case "Backup":
              lmissionHeader = missionHeaderValidate.validateMasterContactNo(lmissionHeader, lLawyerBus.getDebtorContactNo());
              break;
            case "Procedure LJ":
              lmissionHeader = missionHeaderValidate.validateMasterContactNo(lmissionHeader, lmissionHeader.getMasterContactNo());
              break;
            case "Recovery":
              lmissionHeader = missionHeaderValidate.validateManager(lmissionHeader, lLawyerBus.getManager());
              //lmissionHeader = missionHeaderValidate.validateMainInterlocutorNo(lmissionHeader, lLawyerBus.getMainInterlocutorNo());
              lmissionHeader = missionHeaderValidate.validateMasterContactNo(lmissionHeader, lLawyerBus.getDebtorContactNo());
              ...
              lmissionHeader.setRepresentativeofCreditors( lLawyerBus.getRepresentativeofCreditors());
              break;
            default:
              lmissionHeader = missionHeaderValidate.validateMasterContactNo(lmissionHeader, lmissionHeader.getMasterContactNo());
              break;
