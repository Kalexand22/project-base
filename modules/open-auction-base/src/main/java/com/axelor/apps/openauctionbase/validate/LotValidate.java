...
public class LotValidate {
    ...
    @Inject
    public LotValidate(MissionServiceLineRepository missionServiceLineRepo, MissionLineRepository missionLineRepo) {
   ...
   //TESTFIELD("Lot Type", "Lot Type"::Lot); //AP32.ST//
     */
    public Lot validateGroupingAuthorization(Lot lot, Boolean groupingAuthorization) throws AxelorException {
        
        if(lot.getLotType() != null && !lot.getLotType().equals(LotRepository.LOTTYPE_SELECT_LOT)) {
            throw new AxelorException(lot, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR, "Le champ Autorisation de groupement ne peut être coché que si le type de lot est Lot");
        }
   ...
   //OnValidateBEGIN
   //TESTFIELD("Lot Type", "Lot Type"::Lot); //AP32.ST//
    public Lot validateUngroupingAuthorization(Lot lot, Boolean ungroupingAuthorization) throws AxelorException {
        
        if(lot.getLotType() != null && !lot.getLotType().equals(LotRepository.LOTTYPE_SELECT_LOT)) {
            throw new AxelorException(lot, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR, "Le champ Autorisation de dégroupement ne peut être coché que si le type de lot est Lot");
        }
   ...
   ////>>AP52.ST//
    ...
    public Lot validateLotMissionPriceGroup(Lot lot, MissionLotPriceGroup lotMissionPriceGroup) {
        ...
        missionServiceLineList = Beans.get(MissionServiceLineRepository.class)
            .all()
            .filter("self.lotNo = ?1 AND self.transactionType = ?2 AND self.lotPriceGroup != ?3 AND self.invoicedQuantity = 0", 
            lot, MissionServiceLineRepository.TRANSACTIONTYPE_SELECT_MISSION, lotMissionPriceGroup).fetch();

        for(MissionServiceLine missionServiceLine : missionServiceLineList) {
            missionServiceLine = missionServiceLineValidate.validateLotPriceGroup(missionServiceLine, lotMissionPriceGroup);
   ...
   ////>>AP52.ST//
    ...
    public Lot validateLotAuctionPriceGroup(Lot lot, AuctionLotPriceGroup lotAuctionPriceGroup) {
        ...
        missionServiceLineList = Beans.get(MissionServiceLineRepository.class)
            .all()
            .filter("self.lotNo = ?1 AND self.transactionType = ?2 AND self.lotPriceGroup != ?3 AND self.invoicedQuantity = 0", 
            lot, MissionServiceLineRepository.TRANSACTIONTYPE_SELECT_VENTE, lotAuctionPriceGroup).fetch();

        for(MissionServiceLine missionServiceLine : missionServiceLineList) {
            missionServiceLine = missionServiceLineValidate.validateAuctionLotPriceGroup(missionServiceLine, lotAuctionPriceGroup);
            missionServiceLineRepo.save(missionServiceLine);
        }
        return lot;
    }
   ...
   //END;//
    ...
    public Lot validateAddedValueType(Lot lot, String addedValueType) throws AxelorException {
        if(addedValueType != null && !addedValueType.equals(LotRepository.ADDEDVALUETYPE_SELECT_FREE)) {
   ...
   ////>>AP63.ISAT.EBA//
    public Lot validateFreeReasonCode(Lot lot, FreeReason freeReasonCode) throws AxelorException {
        if(freeReasonCode != null && lot.getAddedValueType() != null && !lot.getAddedValueType().equals(LotRepository.ADDEDVALUETYPE_SELECT_FREE)) {
            throw new AxelorException(lot, TraceBackRepository.CATEGORY_INCONSISTENCY, "Le champs Type de plus-value doit être de type Exonéré");
        }
   ...
   ////>>ap49 isat.zw//
    ...
    public Lot validateCurrentAuctionNo(Lot lot, AuctionHeader currentAuctionNo) {
        ...
        if(currentAuctionNo != null) {
            if(lot.getLotGeneralStatus().equals(LotRepository.LOTGENERALSTATUS_SELECT_IDENTIFIED) || lot.getLotGeneralStatus().equals(LotRepository.LOTGENERALSTATUS_SELECT_ONMISSION)) {
