...
public class MissionLineValidate {
    ...
    END;
     */
    public MissionLine validateLotMissionNo(MissionLine missionLine, Integer lotNoMission) throws AxelorException {
        //EVALUATE("Lot Mission No.","Lot No./Mission"); //AP10.ST
        missionLine.setLotNoMission(lotNoMission);
       
        if (this.getCurrentAuctionNo(missionLine) != null) {
            throw new AxelorException(missionLine, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR, "Vous ne pouvez pas modifier le numéro de Lot/Mission car ce lot est déjà affecté à une vente");
        }
    ...
    */
    public MissionLine validateNoLot(MissionLine missionLine, Lot lot)
    {
        ...
        if(missionLine.getType() == null || missionLine.getType().equals(MissionLineRepository.TYPE_SELECT_EMPTY)) {
        ...
        else
        {
            if (lot != null) {
                ...
                lot = lotValidate.validateLotGeneralStatus(lot, LotRepository.LOTGENERALSTATUS_SELECT_ONMISSION);
