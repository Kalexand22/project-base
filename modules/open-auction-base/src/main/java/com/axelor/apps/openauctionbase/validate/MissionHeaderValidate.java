...
public class MissionHeaderValidate {
   ...
   //MissionManagement.SaveChangeStatus(xRec,Rec);//
    ...
    public MissionHeader validateMissionStatus(MissionHeader missionHeader, String missionStatus) throws AxelorException {
        missionHeader.setMissionStatus(missionStatus);
      if (missionHeader.getMissionStatus().equals(MissionHeaderRepository.MISSIONSTATUS_SELECT_FINISHED)) {
         if (!missionManagement.ifAllMissionServiceInvoiced(missionHeader)) {
            throw new AxelorException(missionHeader, TraceBackRepository.CATEGORY_INCONSISTENCY, "Toutes les lignes de mission doivent être facturées");
         }
         if (!missionManagement.ifAllTodoMissionCompleted(missionHeader)) {
            throw new AxelorException(missionHeader, TraceBackRepository.CATEGORY_INCONSISTENCY, "All mission services must be invoiced");
         }
      }
      if (missionHeader.getMissionStatus().equals(MissionHeaderRepository.MISSIONSTATUS_SELECT_CANCELED)) {
   ...
   //  //  MissionManagement.ChangeActivity(xRec."Activity Code To Header","No.",'','',"Mission Template Code",FALSE,0); //AP16
   //  //END;
   //  IF "Activity Code To Header" <> '' THEN BEGIN
   //    MissionManagement.ChangeActivity("Activity Code To Header","No.",'','',"Mission Template Code",TRUE,0); //AP16
   //  END;
   ////END;//
    public MissionHeader validateActivityCodeToHeader(MissionHeader missionHeader, ActivityHeader activityCodeToHeader) {
      missionHeader.setActivityCodeToHeader(activityCodeToHeader);
      
      if (missionHeader.getActivityCodeToHeader() != null) {
        missionManagement.changeActivity( missionHeader.getActivityCodeToHeader(), missionHeader, null, null, missionHeader.getMissionTemplateCode(), true, 0);
      }
      return missionHeader;
    }
   ...
   //OnValidateVAR
   public MissionHeader validateCentralizedCustomerNo(MissionHeader missionHeader, Partner centralizedCustomerNo) {
  ...
  //Master_Contact_No Code20
   //OnValidateVAR
   public MissionHeader validateMasterContactNo(MissionHeader missionHeader, Partner masterContactNo) {
   ...
   //END;//
    public MissionHeader validateResponsibilityCenter(MissionHeader missionHeader, TradingName responsibilityCenter) {
      ...
      if (missionHeader.getResponsibilityCenter() != missionHeader.getResponsibilityCenter()) {
        if (toolsMissionChange.allowRespCenterChange(missionHeader, true)) {
            missionHeader = toolsMissionChange.updateRespCenterChange(missionHeader, missionHeader.getResponsibilityCenter());
        }
      }
      return missionHeader;
    }
   ...
   //ContactLawyerManagement.ValidateContact(ContactType::Manager, Manager);//
    public MissionHeader validateManager(MissionHeader missionHeader, Partner manager) {
      ...
      //contactLawyerManagement.validateContact(ContactRepository.CONTACTTYPE_SELECT_MANAGER, missionHeader.getManager());
      return missionHeader;
    }
